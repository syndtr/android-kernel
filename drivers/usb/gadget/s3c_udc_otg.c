...
#if	defined(CONFIG_USB_GADGET_S3C_UDC_OTG_DMA_MODE) /* DMA mode */
#define OTG_DMA_MODE		1

#elif	defined(CONFIG_USB_GADGET_S3C_UDC_OTG_SLAVE_MODE) /* Slave mode */
...
*/
static int s3c_udc_start(struct usb_gadget_driver *driver,
...
*/
static int s3c_udc_stop(struct usb_gadget_driver *driver)
{
	struct s3c_udc *dev = the_controller;
	unsigned long flags;
...
{
	struct s3c_udc *dev = handler->data;
