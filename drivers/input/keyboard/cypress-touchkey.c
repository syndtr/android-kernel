		...
		const struct i2c_device_id *id)
{
	...
	devdata->dev_class = class_create(THIS_MODULE, "cypress");

	if (IS_ERR(devdata->dev_class))
		pr_err("Failed to create class(cypress)!\n");
	
	ts_key_dev = device_create(devdata->dev_class, NULL, 0, NULL, "control");
