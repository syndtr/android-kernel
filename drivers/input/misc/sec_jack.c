...
struct switch_dev jack_switch = {
	.name		= "h2w",
	.attrs		= jack_switch_attrs,
	.attrs_nr	= ARRAY_SIZE(jack_switch_attrs),
};
...
static void sec_jack_set_type(struct sec_jack_info *hi, int jack_type)
{
	...
	switch_set_state(&jack_switch, JACK_SWITCH_STATE, jack_type);
}
...
void sec_jack_detect_work(struct work_struct *work)
{
	...
	while (time_left_ms > 0) {
		if (!(gpio_get_value_cansleep(hi->pdata->det_gpio) ^ npolarity)) {
			/* jack not detected. */
			handle_jack_not_inserted(hi);
			return;
		}
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	ret = gpio_direction_input(pdata->det_gpio);
	if (ret) {
	...
	ret = switch_dev_register(&jack_switch);
	if (ret < 0) {
	...
	ret = request_any_context_irq(hi->det_irq, sec_jack_detect_irq,
...
err_create_wq_failed:
	...
	switch_dev_unregister(&jack_switch);
